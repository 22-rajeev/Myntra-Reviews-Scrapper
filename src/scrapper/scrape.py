...
class ScrapeReviews:
    ...
    def scrape_product_urls(self, product_name):
        try:
            search_string = product_name.replace(" ","-")   # no_of_products = int(self.request.form['prod_no'])
          

            encoded_query = quote(search_string)  # Navigate to the URL
    ...
    def get_review_data(self) -> pd.DataFrame:
        try:
            ...
            while review_len < self.no_of_products and review_len < self.no_of_products:
            ...
            if not product_details:
                raise CustomException(Exception("No product reviews found"), sys)
            ...
            # Return columns and values safely
